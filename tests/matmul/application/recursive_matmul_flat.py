...
def matmul_block(matA, matB, matC):
    res = None
...
def matmul(matA, matB, matC, tmp_files_path):
    if len(matC) == 1 and len(matC[0]) == 1:
        dependency_tokens = matmul_block(matA, matB, matC)
        return dependency_tokens
    elif len(matA) == 1 and len(matA[0]) == 1:
        dependency_tokens = []
        for j in range(len(matB[0])):
            dependency_token = matmul(matA, as_collection(matB[0][j]), as_collection(matC[0][j]), tmp_files_path)
            dependency_tokens.append(dependency_token)
        return dependency_tokens
    elif len(matB) == 1 and len(matB[0]) == 1:
        dependency_tokens = []
        for i in range(len(matA)):
            dependency_token = matmul(as_collection(matA[i][0]), matB, as_collection(matC[i][0]), tmp_files_path)
            dependency_tokens.append(dependency_token)
        return dependency_tokens
    else:
        result_token_deps = []
        for i in range(len(matC)):
            for j in range(len(matC[0])):
                if len(matB) == 1:
                    return matmul(as_collection (matA[i][0]), as_collection(matB[0][j]), as_collection(matC[i][j]), tmp_files_path)
                else:
                    partials = []
                    dependency_tokens = []
                    for k in range(len(matB)):
                        ...
                        dependency_tokens.append(dependency_token)
                    result_token_dep = addition(partials, as_collection(matC[i][j]), dependency_tokens)
                    result_token_deps.append(result_token_dep)
        return result_token_deps
